...
import random
from sampling.samplers import ULA
from utils.draw_index import draw_index
...
class GaussianMixtureModels(object):
    ...
    def __init__(self, means, weights, batch_size = 10, data_size = 5000):
        """
        One-dimensional gaussian mixture models with constant variance.
        Constructs and samples from a cdf of weighted Gaussians.
        :param means: A list of centers of mass
        :param weights: A list of amplitudes in the length of the means
        :param batch_size:
        :param data_size:
        """
        ...
        super(GaussianMixtureModels, self).__init__()
    ...
    def p(self, x, mu):
        """
        Probability density function of one center of mass
        """
    ...
    def rf(self, theta, batch = None):
        ...
        :param theta: tuple or list of tuples
        :param batch:
        :return:
        """
...
def test_ULA_sampling_3():
    ms = [-4, 4, 19]
    ws = [0.3, 0.2]
    f = GaussianMixtureModels(ms, ws, batch_size=40)
    theta = np.array(ms)
    ula = ULA(f.rf, f.rdf)
